 ...
 * the License.
 */
package de.gishmo.gwt.editor.client.impl;
...
import de.gishmo.gwt.editor.client.Address;
import de.gishmo.gwt.editor.client.AddressEditor;
import de.gishmo.gwt.editor.client.Person;
...
public class DelegateMapTest extends TestCase {
  ...
  private List<Editor<?>> editors(DelegateMap map, Object o) {
    ...
    for (AbstractEditorDelegate<?, ?> delegate : map.get(o)) {
      toReturn.add(getEditor(delegate));
    }
    return toReturn;
  }

  private Editor<?> getEditor(AbstractEditorDelegate<?, ?> delegate) {
    //TODO
    return null;//delegate.getEditor();
  }
